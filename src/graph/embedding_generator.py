...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        logger.info(f"EmbeddingGenerator initialized (model: {self.config.model})")
    ...
    def embed_text(self, text: str) -> Optional[List[float]]:
        ...
        try:
            # Truncate if needed
            if len(text) > self.config.max_tokens_per_text * 4:  # Rough char estimate
    ...
    def embed_chunks(
        ...
        for pattern in ["thread_chunks/*.json", "individual_chunks/*.json",
                       "attachment_chunks/knowledge/*.json"]:
